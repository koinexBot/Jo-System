...
bot.on("message", async message => {
  ...
  if(cmd === `${prefix}warn`){
     ...
     .setDescription(`**Command:** W!warn`)
     .setColor("RANDOM")
     .addField("Description:", `warn a member`)
     .addField("Usage:", `W!warn [user] [reason]`)
     .addField("Example:", `W!warn @iiKillerxSG swear`)
  ...
  if(cmd === `${prefix}help`){
    ...
    .setColor("RANDOM")
    .addField("W!help -" , "كي تشوف اوامر البوت" )
    .addField("W!warn -" , "لكي تعطي احد تحذير" )
    .addField("W!report -" , "لكي تشتكي على احد" )
    .addField("W!تقديم -" , "t8dem لكي تقديم للكلان مفتوح كل خميس و جمعة و احيانا السبت" )
    .addField("W!mute -" , "لكي تعطي احد ميوت" )
    .addField("W!unmute -" , "لكي تشيل من احد ميوت" )
    .addField("W!ban -" , "لكي تشيل بان من احد" )
    .addField("W!bc -" , "لكي ترسل رسالة لكل الناس" )
    .addField("W!suggestion -" , "لكي تقترح شيء نسويه" )
    .addField("W!capitals -" , "لعبة حلوى اسرع واحد يكتب الكلمة" )
    .addField("W!cat -" , "شطحة مو لازم تسويها الا لو تبي" )
    .addField("W!catinfo -" , "شطحة مو لازم تسويها الا لو تبي" )
    .setAuthor(`${message.author.tag}`, message.author.avatarURL)
    .setFooter(bot.user.username , bot.user.avatarURL)
    .setTimestamp();
  ...
  if(cmd === `${prefix}unmute`){
     ...
     .addField("Usage:", `W!unmute [user]`)
     .addField("Example:", `W!unmute @iiKillerxSG`)
     .setAuthor(`${message.author.tag}`, message.author.avatarURL)
     .setFooter(bot.user.username , bot.user.avatarURL)
     .setTimestamp();
...
.setDescription(`**Command:** W!bc`)
.setColor("RANDOM")
.addField("Description:", `bc a message`)
.addField("Usage:", `W!bc [message]`)
.addField("Example:", `W!bc test`)
.setAuthor(`${message.author.tag}`, message.author.avatarURL)
.setFooter(bot.user.username , bot.user.avatarURL)
.setTimestamp();
...
if (cmd === `${prefix}capitals`){
  ...
  msg.channel.send(secondsembed).then(() => {
   ...
   if(cmd === `${prefix}suggestion`){
     ...
     .setDescription(`**Command:** W!suggestion`)
     .setColor("RANDOM")
     .addField("Description:", ` suggestion a idea`)
     .addField("Usage:", `W!suggestion [idea]`)
     .addField("Example:", `W!suggestion test`)
     .setAuthor(`${message.author.tag}`, message.author.avatarURL)
     .setFooter(bot.user.username , bot.user.avatarURL)
     .setTimestamp();
